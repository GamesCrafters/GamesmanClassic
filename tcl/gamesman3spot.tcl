...
#    wm geometry . "+0+0"
    ...
    label .f3.cl -bitmap @$xbmLeft -foreground red
    ...
    label .f3.cr -bitmap @$xbmRight -foreground blue
...
proc DoValueMovesExplanation {} {
	...
	canvas .winValueMoves.f$i.c -width $theMoveSize -height $theMoveSize -background $theColor
...
#    puts "DOBoard called"
 ...
 #        .winBoardControl.f2.redo {left expand fill}
    ...
    if {$kInteractionType == "3DotSpecial"} {
#	canvas .winBoard.c -width 320 -height 320 
	Do_3Dot_Thing .winBoard

    } else {
	#puts "creating generic board"
	canvas .winBoard.c \
...
##
#############################################################################
global ShowingValueMoves
set ShowingValueMoves 0
proc ShowValueMoves {} {
...
proc DrawMove { w theMoveArg color drawBig } {
    ...
    } elseif { $kInteractionType == "3DotSpecial" } {   
    } else {
...
proc DoComputersMove {} {
    ...
    if { $kInteractionType == "3DotSpecial" } {
	HandleTurn; HandleTurnEnd .winBoard
	return
    }
...
proc HandleTurn {} {
	...
	if { $thePrimitive == "Tie" } {
		...
		.winBoardControl.f5.c config -bitmap @$xbmLeft -foreground red
		.winBoardControl.f5.mesTurn config \
		...
		.winBoardControl.f5.c config -bitmap @$xbmRight -foreground blue
		.winBoardControl.f5.mesTurn config \
	...
	if { $tcl_platform(platform) == "macintosh" || \
	    ...
	    if { $thePlayersTurn } {
		.winBoard.c config -cursor "@$xbmLeft red"
	    } else {
		.winBoard.c config -cursor "@$xbmRight blue"
	    }
	}
