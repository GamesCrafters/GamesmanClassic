...
#############################################################################
proc GS_GetOption { } {
    global gMisereGame gForcedCapture gBoardWidth gBoardHeight gAllowHWrap gAllowVWrap

  
    set option 0
#new 
    #set option [expr $gForcedCapture << 1]
    #set option [expr $gAllowHWrap << 2]
    #set option [expr $gAllowVWrap << 3]
    #set option [expr $gBoardHeight << 4]
    #set option [expr $gBoardWidth << 8]
#/new
...
proc GS_SetOption { option } {
   ...
   # set gAllowHWrap [expr [expr option & 0x2] >> 1]
   # set gAllowVWrap [expr [expr option & 0x4] >> 2]
   # set gStandardGame [expr [expr option & 0x8] >> 3]
   # set gBoardHeight  [expr [expr option & 0xf0] >> 4];
   # set gBoardWidth = [expr [expr option & 0xff0] >> 8]
}
