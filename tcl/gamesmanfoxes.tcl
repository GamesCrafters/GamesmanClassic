...
proc CreateArrow { w theMoveArgAbsolute theSlotSize theSlotsX theSlotsY color } {
    ...
    set slotXend [expr $theMoveEnd % $theSlotsX]
    set slotYend [expr $theMoveEnd / $theSlotsX]
	...
	#left, up; right, down
	if { ($slotXend < $slotXbegin && $slotYend < $slotYbegin) || ($slotXend > $slotXbegin && $slotYend > $slotYbegin) } {
	    set theLineX [expr ($theLineLength + $theLineGap) * $theOffsetCenter]
	    set theLineY [expr $theLineX]
	    set theLineOffsetX [expr $theHalfSlot + ($theLineGap * $theOffsetCenter)]
