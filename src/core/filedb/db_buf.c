...
// each record has rec_size bytes, along with one extra byte to keep
// track of valid bit
gamesdb_buffer *gamesdb_buf_init(gamesdb_pageid rec_size,
                                 gamesdb_pageid max_recs,
                                 gamesdb_pageid num_buf) {
    gamesdb_buffer *bufp =
        (gamesdb_buffer *)gamesdb_SafeMalloc(sizeof(gamesdb_buffer));
...
gamesdb_bufferpage *gamesdb_buf_addpage(gamesdb *db) {
    gamesdb_bufferpage *buf =
        (gamesdb_bufferpage *)gamesdb_SafeMalloc(
            sizeof(gamesdb_bufferpage));
    if (buf == NULL) {
        return buf;
    }
    gamesdb_buffer *bufp = db->buffer;
    buf->mem = (char *)gamesdb_SafeMalloc(
        sizeof(char) * bufp->buf_size * bufp->rec_size);
    if (buf->mem == NULL) {
...
void gamesdb_buf_removepage(gamesdb *db) {
    ...
    if (newhead == NULL) {
        printf(
            "db_buf: There is only one page in the pool. Cannot "
            "shrink anymore.");
        return;
    }
...
int gamesdb_buf_flush_all(gamesdb *db) {
    ...
    // a straight squential scan will cause ordered forward access to
    // the db_store this will be as fast as it gets int i;
    gamesdb_buffer *bufp = db->buffer;
    gamesdb_bufferpage *buf;
...
int gamesdb_buf_read(gamesdb *db, gamesdb_frameid spot,
                     gamesdb_pageid vpn) {
    ...
    if (spot->tag == 0)
        // the buffer is uninitialized, this means no record exists in
        // the page
        spot->tag = vpn;
    ...
    if (GAMESDB_DEBUG) {
        ...
        printf("buf_read: spot = %" PRIu64
               ", buf_tag = %llu, mytag = %llu\n",
               (uint64_t)spot, spot->tag, vpn);
#else
        printf("buf_read: spot = %u, buf_tag = %llu, mytag = %llu\n",
               (uint32_t)spot, spot->tag, vpn);
#endif
    }
    return 0;
}
...
int gamesdb_buf_write(gamesdb *db, gamesdb_frameid spot) {
    ...
    if (spot->dirty == GAMESDB_TRUE) {
        ...
        if (GAMESDB_DEBUG) {
#if defined(__LP64__) || defined(_WIN64)
            printf("buf_write: spot = %" PRIu64 ", buf_tag = %llu\n",
                   (uint64_t)spot, spot->tag);
#else
            printf("buf_write: spot = %u, buf_tag = %llu\n",
                   (uint32_t)spot, spot->tag);
#endif
        }
    }
