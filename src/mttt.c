...
CONST_STRING kHelpExample = "         ( 1 2 3 )           : - - -\n\
LEGEND:  ( 4 5 6 )  TOTAL:   : - - - \n\
         ( 7 8 9 )           : - - - \n\n\
Computer's move              :  3    \n\n\
         ( 1 2 3 )           : - - X \n\
...
** FLIP						ROTATE
**
** 0 1 2	2 1 0		0 1 2		6 3 0		8 7 6
*2 5 8
** 3 4 5  ->    5 4 3		3 4 5	->	7 4 1  ->	5 4 3	->
*1 4 7
** 6 7 8	8 7 6		6 7 8		8 5 2		2 1 0
*0 3 6
*/
...
void DebugMenu() {
  ...
  do {
    ...
    case 'h':
      ...
      break;
    case 'C':
    case 'c': /* Write PPM to s(C)reen */
      // tttppm(0,0);
      break;
    case 'I':
    case 'i': /* Write PPM to f(I)le */
      // tttppm(0,1);
      break;
    case 'S':
    case 's': /* Write Postscript to (S)creen */
      // tttppm(1,0);
      break;
    case 'F':
    case 'f': /* Write Postscript to (F)ile */
      // tttppm(1,1);
      break;
    case 'B':
    case 'b':
...
extern void gPenHandleTclMessage(int options[], char *filename,
                                 Tcl_Interp *tclInterp, int debug);
...
void PrintPosition(POSITION position, STRING playerName, BOOLEAN usersTurn) {
  ...
  printf("\n         ( 1 2 3 )           : %s %s %s\n",
         gBlankOXString[(int)theBlankOx[0]], gBlankOXString[(int)theBlankOx[1]],
         gBlankOXString[(int)theBlankOx[2]]);
  printf("LEGEND:  ( 4 5 6 )  TOTAL:   : %s %s %s\n",
         gBlankOXString[(int)theBlankOx[3]], gBlankOXString[(int)theBlankOx[4]],
         gBlankOXString[(int)theBlankOx[5]]);
  printf("         ( 7 8 9 )           : %s %s %s %s\n\n",
         gBlankOXString[(int)theBlankOx[6]], gBlankOXString[(int)theBlankOx[7]],
...
BOOLEAN ThreeInARow(BlankOX *theBlankOX, int a, int b, int c) {
  return (theBlankOX[a] == theBlankOX[b] && theBlankOX[b] == theBlankOX[c] &&
          theBlankOX[c] != Blank);
}
...
void PositionToAutoGUIString(POSITION position,
                             char *autoguiPositionStringBuffer) {
  BlankOX oxboard[BOARDSIZE];
  ...
  board[BOARDSIZE] = '\0';
  AutoGUIMakePositionString(WhoseTurn(oxboard) == x ? 1 : 2, board,
                            autoguiPositionStringBuffer);
}

void MoveToAutoGUIString(POSITION position, MOVE move,
                         char *autoguiMoveStringBuffer) {
  BlankOX oxboard[BOARDSIZE];
  PositionToBlankOX(position, oxboard);
