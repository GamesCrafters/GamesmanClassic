...
void MoveToString(MOVE move, char *moveStringBuffer) {
    if (move == ~0ULL) {
        snprintf(moveStringBuffer, MAX_MOVE_STRING_LENGTH, "Pass");
        return;
    }
    int move_idx = -1;
    ...
    snprintf(moveStringBuffer, MAX_MOVE_STRING_LENGTH, "%d", move_idx + 1);
    return;
}
...
static uint8_t *read_gzip_file_to_memory(const char *filename, size_t *out_size) {
    *out_size = 0;
    ...
    while (1) {
        ...
        int bytes_read = gzread(
            gz,
            buffer + *out_size,
            (unsigned int)(capacity - *out_size)
        );
...
static int read_metadata_gz(uint64_t *W_out) {
    ...
    if (!metadata) {
        printf("Error: Metadata gzip read error\n");
        return 0;
    }

    /*
     * Your solver wrote W as an int.
     * Your old code read sizeof(uint64_t)/2 = 4 bytes.
     */
    if (metadata_size < sizeof(uint32_t)) {
        printf("Error: Metadata file too small\n");
        free(metadata);
        return 0;
    }

    uint32_t W32 = 0;
    memcpy(&W32, metadata, sizeof(uint32_t));
    *W_out = (uint64_t)W32;
...
static int read_offsets_for_tier_gz(
    uint8_t tier,
    uint64_t W,
    uint64_t *comp_data_offsets,
    uint64_t *comp_idx_offsets
) {
    ...
    if (!offsets) {
        printf("Error: Offset gzip read error\n");
        return 0;
    }

    uint64_t tier_idx = (tier - 4) * 2 * (W * sizeof(uint64_t));
    uint64_t need_bytes = tier_idx + 2 * W * sizeof(uint64_t);

    if (offsets_size < need_bytes) {
        printf("Error: Offset gzip file too small\n");
        free(offsets);
        return 0;
    }

    memcpy(comp_data_offsets, offsets + tier_idx, W * sizeof(uint64_t));
    memcpy(comp_idx_offsets, offsets + tier_idx + W * sizeof(uint64_t), W * sizeof(uint64_t));
...
UINT64 GetInfoFromBlobFile(POSITION p, FILE *f) {
    ...
    if (!read_offsets_for_tier_gz(tier, W, comp_data_offsets, comp_idx_offsets)) {
        return 0;
    }
    ...
    fseek(f, comp_data_offsets[owner], SEEK_SET);

    /*
     * Read gzipped tier.idx into memory.
    ...
    if (!record_map) {
        printf("Error: Record gzip file open/read error\n");
        return 0;
    }

    if (comp_idx_offsets[owner] >= (uint64_t)record_map_size) {
    ...
    uint64_t comp_idx_size = (comp_idx_offsets[owner] == array_max(comp_idx_offsets, W))
        ? record_map_size - comp_idx_offsets[owner]
