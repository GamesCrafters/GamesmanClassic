...
solverdata* initializesolverdata(int keylen, int maxtier);

/*Inserts the given (key,val) pair to a solver, at the given tier. It is assumed that any insert either occurs only once
or occurs with the same value for a given key, and assumes that val != 0*/
void solverinsert(solverdata* data, uint64_t key, char val, char tier);


/*Reads a data value at the given key, returning 0 if the value has not been inserted*/
char solverread(solverdata* data, uint64_t key, char tier);


/*Writes to the given folder the current solver state.
...
void solversave(solverdata* data, char* foldername);
...
char playerread(playerdata* data, uint64_t key, char tier);
