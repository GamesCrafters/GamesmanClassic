...
VALUE DetermineValueBPDBSTD(POSITION position)
{
        ...
        if(!gBitPerfectDB) {
            ...
            BPDB_TRACE("DetermineValueBPDBSTD()", "Bit-Perfect DB must be the selected DB to use the slices solver", status);
        ...
        status = AddSlot( 1, "MEX", TRUE, TRUE, FALSE, &MEXSLOT );              // slot 2
        if(!GMSUCCESS(status)) {
        ...
        status = Allocate();
        if(!GMSUCCESS(status)) {
            BPDB_TRACE("DetermineValueBPDBSTD()", "Could not allocate database", status);
            goto _bailout;
        }
        ...
        gameval = DetermineValueBPDBSTDHelper( position );
...
VALUE DetermineValueBPDBSTDHelper( POSITION position )
{
    ...
    /* It's been seen before and value has been determined */
		...
		child = gCanonicalPosition(child);
            ...
            value = DetermineValueBPDBSTDHelper(child);       /* DFS call */
