                ...
                for move in position.generate_moves():
                    ...
                    if not child.is_illegal():
                        ...
                        if hash(child) not in self.seen:  # first time we've seen it
                            ...
                            self.from_board[hash(child)] = [position]
                        elif self.seen[hash(child)] == level+1 and (position not in self.from_board[hash(child)])  : # another sol path!
                            
                            self.move[hash(child)].append(position.reverse_move(move))
                            self.from_board[hash(child)].append(position)
                            #print "move["+str(hash(child))+"].append w/ "+str(position.reverse_move(move))
                        else:
        ...
        for level in self.levels.keys():          
            if level != 0:                              # don't include solutions
                ...
                for p in self.levels[level]:
                    for board in self.from_board[hash(p)]:
#                        answer = p + move
