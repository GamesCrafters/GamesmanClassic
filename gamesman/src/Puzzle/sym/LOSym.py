...
#            boards+=[self.do_move(move)]
#        for me in range(len(boards)):
#            for you in range(len(boards)):
#                if boards[me] and boards[you] and me != you:
#                    if boards[me].my_con_board().board == boards[you].my_con_board().board:
#                        if hash(boards[me]) < hash(boards[you]):
#                            boards[you] = []
#                        else:
#                            boards[me] = []
...
#        return moves
    ...
    def do_move(self, move):
        ...
        if posx > 0:
            board[size * posy + (posx - 1)] = int(not board[size * posy + (posx - 1)])
        if posy > 0:
            board[size * (posy - 1) + posx] = int(not board[size * (posy - 1) + posx])
        if posx < size-1:
            board[size * posy + (posx + 1)] = int(not board[size * posy + (posx + 1)])
        if posy < size-1:
            board[size * (posy + 1) + posx] = int(not board[size * (posy + 1) + posx])
    ...
    def con_board(self):
        size = int(sqrt(self.board.__len__()))
        boardlist = [self.board]
        rotboard = list(self.board)
        for i in range(4):
    ...
    def __str__(self):
        ...
        return toreturn.__getslice__(0,toreturn.__len__()-1)
