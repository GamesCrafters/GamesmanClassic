...
// may need to change TIER scheme to reflect what is on notes with tier = buckets + 5*small + 25*large
...
void SetupTierStuff() {
  ...
  for(; tier <= 12; tier++) {  // 12 total pieces - 4 large, 4 small, 2 redB, 2blueB
    LpiecesArray[1] = 9 - tier


    pieces_array[1] = 0;
    pieces_array[2] = tier; // large

    pieces_array[4] = 0;
    pieces_array[5] = tier; // small

    pieces_array[7] = boardsize - tier;
    pieces_array[8] = boardsize - tier; // redB

		generic_hash_init(boardsize, pieces_array, NULL, 0);
		generic_hash_set_context(tier);  // sets context for this generic hash
	}
	
	// initial position stuff (he copied it over)
	int i;
