...
void printBoard (char *board) {
    ...
    printf("\n   ");
    for (j = 0; j < BOARD_COLS; j++)
	printf("%d ", j+1);  /*start count from 1*/
    printf("\n");
}
...
void PrintComputersMove (MOVE computersMove, STRING computersName)
{
    ...
    printf ("%s moves the piece %c%d %s\n", computersName, \
	    Row(position)+ROW_START, Column (position), directions[direction]);
}
...
void PrintMove (MOVE move)
{
    ...
    printf ("[%c%d %s]", Row (position)+ROW_START, Column (position) + 1, \
...
BOOLEAN ValidTextInput (STRING input)
{
    ...
    if (input[2] != ' ')
	return FALSE;
    if ((input[0] < ROW_START) || (input[1] > (ROW_START+BOARD_ROWS)) ||
	(input[1] < '1') || (input[1] > '1'+BOARD_COLS))
	return FALSE;
...
void setOption (int option)
{
    ...
    /* InitializeGame	(); needed if and only if the size changes */
}
