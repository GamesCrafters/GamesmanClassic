...
\texttt{/data/mgame\_opt\_tier.dat.gz}
...
\textbf{Description}\\
\texttt{WriteLevelFile} reads an array of hash values and returns a level file. This file may be of any format type. The function optimizes for size by generating all types sequentially and comparing size of file after each file is generated. If the last value in the file is not a solitary '1' on the last line (preceeded by a 0x12) then the file is corrupted. The array of input hash values is in the form of a BITARRAY. A BITARRAY basically has each bit be a 1 if the position is reachable and a 0 if the position is not reachable.\\
\textbf{Arguments}\\
